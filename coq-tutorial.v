...
(* First of all, let's download and install CoqIDE. You can run this
file in it.

For Windows and MacOS users, you can download from here,
https://www.mathstat.dal.ca/~xbian/uploads/softwares/

For Linux users, please run this command in your terminal: sudo snap
install coq-prover
...
(* We start with data types, and along the way, we also pick up some
Coq basics.  For now, we just regard "type" as "set". Integers have
type Int, and the conversely, the type Int is the set of all integers.
*)

(* Empty type or Empty set *) 
Inductive Empty : Set := .

(* In CoqIDE, press Ctrl + rightarrow to type-check things defined
till the cursor, Ctrl + downarrow to type-check the next clause, Ctrl
+ uparrow to type-check the previous clause, and Ctrl + End to
type-check the whole file. *)
...
(* We want to show add is associative, i.e., forall (a b c), a + (b +
c) = (a + b) + c. *)
...
defined in Coq is total and hence forall quantified. This suggests
that we should define associativity as a fucntion on a, b, c, which
returns the "equal" property. E.g., something like this, *)

(*
Definition assoc : Nat -> Nat -> Nat -> Equality Property
*)

(* Instead, let use reserved word Theorem instead of Definition. In
Coq, there is not much difference between the following keywords:
Defintion; Lemma; Theorem; Example; Remark; Fact. The difference is
mainly for documentation. Coq may treat them slightly differently, but
not a lot.  *)
...
(* First try, since there is two possibilty, either equal or not
equal, maybe lets use Bool to indicate this. We can define a function
tatking two natural numbers, and if they are equal, it return "true",
otherwise "false".  *)
...
(* Again, we need equality for Bool. Like for Nat, we can define
equalilty test for Bool, easier than for Nat.  *)
...
mean by "always equals to"? The equality test functions we defined
...
We know a fucntion "always" return a value when fed with an input. We
...
Given natural numbers a b, we want a = b to be a type! not a test
...
Recall, Empty is such that there is no function to it (excepty from
...
(* We are still using the test function, but we should be smarter than
that. We have the equivalent defintion without using the test
fucntion.  *)

Inductive Equal (a : Nat) : Nat -> Set := 
  | refl : Equal a a.

(* This type reads: 1) there is only one element in Equal a a for each
a in Nat, so like Singleton, but not exactly Singleton; 2) there is no
elment in Equal a b provided a is not equal to b, which is Empty.  *)
...
(* A proof of this theorem is just a fucntion from Nat x Nat x Nat to
the type a + (b + c) == a + (b + c). Let's contruct it.  *)
Proof. 
intros a b c.
induction a as [ | a' ih].
...
other lauguage, it might be called Vector. One problem with that,
vector or array always has a size or length, but we might have more
things to sort than that length limitation.

In Coq, or Agda, Haskell, or any other language that supports infinite
...
(* Apparently, we are sorting according which is larger and which is
smaller, we want to sort them from small to large. We are facing the
same problem when we define equality. We need more than just a testing
function. We want something like 1 < 2 being a set, a type, especially
a singleton set. Very similar to equality, we define LessOrEqual as
follows: *)
...
covluted. The main techniuque use here is "dependent pattern
...
Recall in boolean logic, given a boolean variable b, (b => false) =
true iff b = false, and (b => false) = false iff b = true. In other
word, not b = b => false.

If A is a Set, then (A -> Empty) is the set of functions from A to
Empty. A -> Empty is empty iff A is not Empty, and A -> Empty is not
empty iff A is Empty. We will define Not A to be A -> Empty.

The idea behind this is what is called the "Brouwer-Heyting-Kolmogorov
intepretation" or "BHK interpretation" of intuitionistic logic, or
"Curry-Howard correspondence". Roughly speaking, we can regard a set
or a type as a proposition and elments as proofs. E.g., a == a is a
type and a proposition that a is equal to a, and refl is an element of
a == a, and also a proof.

Back to the negation of a set A. If A is empty, or has no proof, then
Not A should have a proof, i.e., there is one function in A ->
Empty. On the other hand, if A has a proof, then Not A shouldn't have
any proof, i.e. A -> Empty is empty.
...
(* Have defined order on Nat and all kinds of properties of order. We
...
(* We also want the sorted list to be a permutation of the orginial
list, or equivalently, we want two lists have the same elements
(counting duplicates).*)
